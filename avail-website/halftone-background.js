(function () {
  ...
  // Stable per-position pseudo-random value in [0, 1)
  function hash2(x, y) {
  ...
  function renderFlow(ctx, c, time, p) {
    ...
    for (i = 0; i < c.gridPositions.length; i++) {
      ...
      if (combined > p.threshold) {
        ...
        // Scatter a stable subset of fully-opaque, full-strength "vivid" pixels.
  ...
  function initHalftoneBackground(container, options) {
    var canvas = container.querySelector('canvas');
    if (!canvas) return;

    options = options || {};
    var vividChance = options.vividChance || 0;
